 ...
 */
export async function hasDiaryReminderToday() {
  ...
  try {
    ...
    console.error('일기 리마인더 확인 오류:', error)
    return false
  }
}
 ...
 */
export async function markDiaryReminderShown() {
  const userId = await getCurrentUserId()
  if (!userId) {
    return
  }
  ...
  try {
    ...
    if (error) {
      ...
      console.error('일기 리마인더 기록 오류:', error)
      // 에러를 던지지 않고 조용히 처리
      return
    }
  } catch (error) {
    ...
    if (error.status !== 406) {
      console.error('일기 리마인더 기록 실패:', error)
    }
  }
}
