...
/**
 * 무통장 충전 신청 제출
 * @param {{ depositorName: string, depositAmountKrw: number, requestedTokens: number, userEmail?: string }} params
 */
export async function submitTokenPurchaseRequest(params) {
  ...
  const requestedTokens = Number(params.requestedTokens)
  ...
  if (error) {
    console.error('토큰 충전 신청 오류:', error)
    throw error
  }
...
/**
 * 관리자: 완료 처리 — 이메일로 사용자를 찾아 신청 토큰을 잔액에 추가
 * @param {string} requestId
 * @returns {Promise<{ request: Object, addedTokens: number, newBalance: number, email: string | null }>}
 */
