...
import {
  ...
  getAbroadSouvenirItems,
  updateAbroadSouvenirItem,
...
export default function TravelItinerarySouvenirList({ tripId }) {
  ...
  const loadItems = useCallback(async () => {
    setIsLoading(true)
    try {
      const data = await getAbroadSouvenirItems(tripId)
      setItems(data)
    } catch (error) {
      showToast(error?.message || '기념품을 불러오지 못했습니다.', TOAST_TYPES.ERROR)
    } finally {
      setIsLoading(false)
    }
  }, [tripId])
  ...
  const handleAdd = async (event) => {
    ...
    try {
      ...
      showToast(error?.message || '기념품 추가에 실패했습니다.', TOAST_TYPES.ERROR)
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const handleToggle = async (item) => {
    ...
    try {
      await updateAbroadSouvenirItem(item.id, { isChecked: nextChecked })
    } catch (error) {
      setItems((prev) =>
  ...
  const saveEdit = async () => {
    ...
    if (!trimmed) {
      showToast('기념품 이름을 입력해주세요.', TOAST_TYPES.ERROR)
      return
    }

    setIsSaving(true)
    try {
      const updated = await updateAbroadSouvenirItem(editingId, { title: trimmed })
      setItems((prev) => prev.map((row) => (row.id === editingId ? updated : row)))
      setEditingId(null)
      setEditingTitle('')
      showToast('기념품을 수정했습니다.', TOAST_TYPES.SUCCESS)
    } catch (error) {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await deleteAbroadSouvenirItem(item.id)
      setItems((prev) => prev.filter((row) => row.id !== item.id))
      showToast('기념품을 삭제했습니다.', TOAST_TYPES.SUCCESS)
    } catch (error) {
