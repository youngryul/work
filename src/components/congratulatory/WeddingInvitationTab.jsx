...
import {
  getWeddingInvitationRecipients,
  saveWeddingInvitationRecipient,
  updateWeddingInvitationRecipient,
  deleteWeddingInvitationRecipient,
 ...
 * 청첩장 인원 탭 컴포넌트
 */
export default function WeddingInvitationTab() {
  ...
  const loadRecipients = async () => {
    try {
      setLoading(true)
      const data = await getWeddingInvitationRecipients()
      setRecipients(data)
    } catch (error) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingRecipient) {
        await updateWeddingInvitationRecipient(editingRecipient.id, dataToSave)
        showToast('인원 정보가 수정되었습니다.', TOAST_TYPES.SUCCESS)
      } else {
        await saveWeddingInvitationRecipient(dataToSave)
  ...
  const handleDelete = async (recipientId) => {
    ...
    try {
      await deleteWeddingInvitationRecipient(recipientId)
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="text-base font-sans text-gray-700">총 {recipients.length}명</div>
        <button
          onClick={() => {
            resetForm()
            setShowForm(true)
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="text-xl font-sans font-bold text-gray-800 mb-2">
